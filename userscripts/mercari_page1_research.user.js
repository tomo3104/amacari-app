...
(function () {
    ...
    const STATIC_MAKERS = [
        ...
        // 2026-08-28追加：manufacturersシートにあるがこのスクリプトの内蔵リストに
        // 反映されていなかった9件（新規メーカー4件＋新設カテゴリ5件）。このRTスクリプトは
        // 他ルートと違いmanufacturersシートを動的に読まず内蔵リストを使うため、
        // シート側に追加しても自動では反映されない（手動で都度追記が必要）。
    ...
    window.addEventListener('DOMContentLoaded', () => {
        // 2026-09-13修正：以前はここでページ読み込みのたびに無条件でmakersを
        // 先読みしていた（RTボタンを押した瞬間の体感速度のための最適化）。しかし
        // このスクリプトは@matchが全ページ（https://jp.mercari.com/*）のため、
        // 発掘リサーチ等の他のクローラーが多数のページを高速に遷移する間も
        // 毎回無関係にこの先読みが発火し、遷移の速さに負けて接続エラーになる
        // ことが頻発していた（実害はないが、サーバーコンソールに大量の
        // 「メーカーリスト取得失敗」ログが出て紛らわしいとユーザーが発見）。
        // 実際にRTを開始する各経路（759・767・787行目付近）には元々
        // 「無ければその場で取得する」フォールバックが用意されているため、
        // ここでの先読みを削除してもRT自体の動作に支障はない。
        startWatchdog();
