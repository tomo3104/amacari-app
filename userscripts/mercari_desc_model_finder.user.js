...
(function () {
    ...
    // 2026-09-03：サーバー側(asin-tools/server.pyのextract_model())の型番抽出を実データ全件検証
    // (desc_experiment_log.jsonl・58,710件)で緩和したところ、旧字だけの判定だとサーバー側が
    // 拾えるはずの型番のうち61.6%がこのスクリプト側では「タイトルに型番なし」と誤判定され、
    // 本来不要な説明文取得（低速なiframe巡回）に回されていたと判明。サーバー側と同じロジック
    // （英字1文字開始・数字開始を許容し、英字のみ／4文字以下は除外）に統一し、無駄な説明文
    // 取得を減らす（発掘リサーチの「1ページで3時間」問題の効率化にも寄与する見込み）。
    const HAS_MODEL_RE = /\b[A-Z0-9][A-Z0-9\-\/\.]{2,}\b/i;
    function hasModelInTitle(title) {
        const text = _normalizeDash(title).toUpperCase();
        const matches = text.match(new RegExp(HAS_MODEL_RE.source, 'g')) || [];
        return matches.some(m => !/^[A-Z]+$/.test(m) && m.length >= 5);
    }
