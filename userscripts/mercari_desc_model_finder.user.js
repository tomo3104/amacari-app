...
(function () {
    ...
    const DESC_LABEL_RE = /(?:【)?(?:型番|品番|型式|形式|型名|モデル(?:番号|名)?|製品番号|商品番号)(?:】)?[：:\s]+[#＃]?((?:[0-9]{1,4}[\-_\/])?[A-Za-z][A-Za-z0-9\-\/\.]{3,24}(?:\s[A-Za-z0-9]{1,10})?)/gi;
    ...
    function _isValidModel(s) {
        // 2026-08-30：DESC_LABEL_REに数字接頭辞・スペース区切り続きの許容を追加した分、
        // 結合後の文字列が長くなるケースがあるため上限を25→35に緩和。
